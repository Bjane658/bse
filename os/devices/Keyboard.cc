...
Key Keyboard::key_hit () {
    ...
    Key some;
    char ctlRegister = ctrl_port.inb();
    char ctlOUTB = ctlRegister & 0x1;
    while(ctlOUTB == 0){
        ctlRegister = ctrl_port.inb();
        ctlOUTB = ctlRegister & 0x1;
        
    }
    char ctlAUXB = ctlRegister & 0x5;
    if(ctlAUXB == 1) return some;

    code = data_port.inb();
    int decoded = key_decoded();
    if(decoded == 1){
        some.ascii(65);
        return gather;
    }
